...
class NewsService:
    ...
    def refresh_news_from_provider(self, db: Session, ticker: str, limit: int = 5) -> list[NewsItem]:
        ...
        items = self.news_provider.fetch_news(
            ticker=ticker,
            instrument_name=instrument.name,
            extra_aliases=alias_values,
            max_items=limit
        )
    ...
    def build_news_context(self, db: Session, user_text: str, resolved_instrument: dict | None = None) -> dict | None:
        ...
        self.refresh_news_from_provider(db, ticker=ticker, limit=5)
